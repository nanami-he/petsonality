#!/usr/bin/env bash
# mbti-pet popup manager -- create/destroy tmux popup overlay
#
# Usage:
#   popup-manager.sh start   -- open pet popup (bottom-right corner)
#   popup-manager.sh stop    -- close pet popup
...
# Compute popup width from pet data (widest of: art, name, stars+rarity)
compute_art_width() {
  [ -f "$STATE_FILE" ] || return
  local name rarity
